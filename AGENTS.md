...
- Parser and scanner logic also follows organization guidance that lives in the [z-shell/.github repository](https://github.com/z-shell/.github), not here.
  The manifest `.github/instruction-surfaces.json` there routes it by task class; a runtime working in this repository opens each file from a `z-shell/.github` checkout before editing `internal/parse/`:
