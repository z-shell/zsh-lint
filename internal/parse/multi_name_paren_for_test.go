...
// The brace-body path masks the extra names before re-parsing, and that mask is
// what makes the rewritten `for a in 1 2; do ... done` header legal. Assert the
// scanner emits it, so removing the mask fails here rather than only in an
// end-to-end parse.
func TestMultiNameParenForMasksExtraNames(t *testing.T) {
	src := []byte("for alpha beta (one two) { : }\n")
	edits, ok := scanForEdits(src, 0)
	if !ok {
		t.Fatal("scanForEdits did not recognise the multi-name paren loop")
	}
	var masked string
	for _, edit := range edits {
		if edit.kind == editMaskExtraNames {
			masked = string(src[edit.start:edit.end])
		}
	}
	if masked != "beta" {
		t.Errorf("masked extra names = %q, want %q", masked, "beta")
	}
}

// scanForExtraNames is the new recognition step, so test it directly: a wrong
// extent would mask real source, and `in`/`do` end the name list.
func TestScanForExtraNames(t *testing.T) {
	cases := []struct {
		src        string
		wantMasked string
	}{
	...
	for _, tc := range cases {
		t.Run(tc.src, func(t *testing.T) {
			// Skip the first name exactly as scanForSite does.
			from := 0
			for from < len(tc.src) && isIdentByte(tc.src[from]) {
				from++
			}
			from = skipSpaces([]byte(tc.src), from)
			start, end, _ := scanForExtraNames([]byte(tc.src), from)
			if got := tc.src[start:end]; got != tc.wantMasked {
				t.Errorf("masked %q, want %q", got, tc.wantMasked)
			}
		})
	}
}
