...
211 fixtures compared (`internal/survey/testdata` and `internal/parse/testdata`), 1 verdict change: the new `ok-flag-pattern-command-substitution.zsh`, which fails on base and passes on the branch.
...
Six mutations, each written to compile and to actually change behavior, all caught:
